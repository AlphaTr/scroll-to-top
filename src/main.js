...
(function (window, chrome, undefined) {
    var style = function (pos, type, color, opacity) {
        var getURL = chrome.runtime.getURL,
            ...
            css = ' #alpha-sttb {width: 50px; height: 50px; position: fixed; z-index: 10000000; {pos}{type1}overflow: hidden; text-align: center; font-family: "alpha-sttb"; speak: none; font-style: normal; font-weight: normal; font-variant: normal; text-transform: none; -webkit-font-smoothing: antialiased; cursor: pointer; -webkit-transition-duration: .4s; -webkit-transform:rotate(180deg); opacity: .1;} #alpha-sttb.rotate {-webkit-transform:rotate(0deg);} #alpha-sttb.fade {opacity: {opacity}; } #alpha-sttb:hover {opacity: .8; } #alpha-sttb:after {{type2} font-size: 50px; line-height: 50px; color: {color};}';
    ...
    chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
        if (request.action && request.action === 'get') {
            if (sender.tab && sender.tab.id) {
                chrome.tabs.insertCSS(sender.tab.id, {code: style('br', 0, '#789', 0.4), allFrames: true}, function () {
                    sendResponse({
                        action: 'render',
